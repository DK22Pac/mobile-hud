...
void MobileLoadingScreen::RenderSplash() {
    ...
    if (*(bool*)0xBAB31C) {
        ...
        if (!*(int*)0x8D093C) {            
            ...
            mobileTex.m_aSplashes[loadSc.m_lCurrentSprite].Draw(SCREEN_COORD(0.0f), SCREEN_COORD(0.0f), SCREEN_WIDTH, SCREEN_HEIGHT, CRGBA(255, 255, 255, 255));
        }
    }
}
...
void MobileLoadingScreen::RenderLoadingBar(float x, float y, unsigned short width, unsigned char height, float progress, signed char progressAdd, unsigned char drawPercentage, unsigned char drawBlackBorder, CRGBA color, CRGBA addColor) {
    float distance = 18.8f;
    CSprite2d::DrawBarChart(SCREEN_COORD(distance), SCREEN_COORD_BOTTOM(28.2f), RsGlobal.maximumWidth - SCREEN_COORD(distance * 2), SCREEN_MULTIPLIER(18.9f), progress, progressAdd, drawPercentage, drawBlackBorder, color, addColor);
}